...
export default {
  ...
  beforeCreate() {
    ...
    this.$root.$on("onPsychoRegister", (age, diplomaNumber, workExperience) => {
      if (age === "") {
        this.$root.$emit("onRegisterValidationError", "Age is required");
        return;
      }
      if (diplomaNumber === "") {
        this.$root.$emit("onRegisterValidationError", "Diploma number is required");
        return;
      }
      if (workExperience === "") {
        this.$root.$emit("onRegisterValidationError", "Work experience is required");
        return;
      }

      const userId = this.user.id;
      const url = `/api/1/users/registerPsychologist?userId=${userId}`;

      const formData = new FormData();
      formData.append('age', parseInt(age));
      formData.append('experience', parseInt(workExperience));
      formData.append('diplomaNumber', parseInt(diplomaNumber));
